document.addEventListener('DOMContentLoaded', function() {
    ...
    albumContainers.forEach(container => {
      ...
      const itemWidth = items[0].offsetWidth + 15; // 包括间隙
      ...
      if (prevBtn) {
        prevBtn.addEventListener('click', () => {
          ...
          const newPosition = Math.min(currentPosition + itemWidth * 3, 0);
      ...
      if (nextBtn) {
        nextBtn.addEventListener('click', () => {
          ...
          const newPosition = Math.max(currentPosition - itemWidth * 3, maxScroll);
          scrollToPosition(newPosition);
          startAutoScroll();
        });
      }
